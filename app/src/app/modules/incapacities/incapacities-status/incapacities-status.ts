...
@Component({
  ...
  templateUrl: './incapacities-status.html',
  styleUrls: ['./incapacities-status.scss'],
})
export class IncapacitiesStatus implements OnInit {
  ...
  loadIncapacities() {
    this.incapacityService.getMyIncapacities().subscribe({
      next: (data) => {
        // Aquí pasamos los datos reales a tu función de procesamiento
